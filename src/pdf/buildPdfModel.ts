...
/**
 * Agrupa las filas en tarjetas, una por SEMANA.
 *
 * La semana es la civil ISO: de lunes a domingo. Por eso un sábado va con el
 * lunes ANTERIOR —el de su propia semana— y no con el lunes siguiente, que ya
 * pertenece a la semana de después. En un mes que abre en sábado eso deja ese
 * primer sábado solo en su tarjeta, y es lo correcto: la tarjeta representa una
 * semana, no una pareja de fechas cualesquiera.
 *
 * El criterio es la semana ISO, no la distancia entre fechas ni el nombre del
 * día. Así el emparejado no se desalinea si falta una fecha (un sábado de
 * asamblea, por ejemplo) —la fecha suelta se queda sola en la tarjeta de su
 * semana— y sigue funcionando con otros días de reunión, sin que este módulo
 * tenga que saber qué es un sábado.
 *
 * `MAX_FILAS_POR_TARJETA` manda por encima de la semana: si el administrador
 * configura tres días de reunión, la semana se parte en dos tarjetas antes que
 * dibujar una tarjeta de tres filas, para la que no está diseñada.
 */
export function groupIntoCards(rows: readonly PdfDateRow[]): PdfWeekCard[] {
  const cards: PdfWeekCard[] = [];
  let current: PdfDateRow[] = [];
  let semanaActual: string | null = null;

  for (const row of rows) {
    const semana = isoWeekKey(fromIso(row.date));
    const cabeEnLaActual = semana === semanaActual && current.length < MAX_FILAS_POR_TARJETA;
